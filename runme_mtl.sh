...
# Train
CUDA_VISIBLE_DEVICES=$GPU_ID python pytorch/main_mtl.py train --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --train_source='curated_and_noisy' --segment_seconds=$SEGMENT_SECONDS --hop_seconds=$HOP_SECONDS --pad_type=$PAD_TYPE --holdout_fold=$HOLDOUT_FOLD --model_type=$MODEL_TYPE --batch_size=$BATCH_SIZE --cuda

# Inference on validation data
CUDA_VISIBLE_DEVICES=$GPU_ID python pytorch/main_mtl.py inference_validation --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --train_source='curated_and_noisy' --segment_seconds=$SEGMENT_SECONDS --hop_seconds=$HOP_SECONDS --pad_type=$PAD_TYPE --holdout_fold=$HOLDOUT_FOLD --model_type=$MODEL_TYPE --iteration=20000 --batch_size=$BATCH_SIZE --cuda
