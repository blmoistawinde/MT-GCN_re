...
class DataGeneratorMT(Base):
    ...
    def generate_train(self):
        ...
        while True:
            ...
            batch_source = np.array([float(x == 'curated') for x in batch_source])
    ...
    def generate_validate(self, data_type, target_source, max_iteration=None):
        ...
        while True:
            ...
            if target_source == "curated":
                batch_source = np.ones(batch_feature.shape[:1])
            else:
                batch_source = np.zeros(batch_feature.shape[:1])
...
class TestDataGeneratorMT(Base):
    def __init__(self, test_feature_hdf5_path, segment_seconds, hop_seconds, 
        pad_type, scalar, batch_size, seed=1234):
        '''Data generator for testing. 
        ...
        self.test_segment_metadata_dict = \
            self.get_segment_metadata_dict(
                self.test_data_dict, test_audio_indexes, self.segment_frames, 
                self.hop_frames, source='curated')
    ...
    def generate_test(self):
        ...
        while True:
            ...
            batch_source = segment_metadata_dict\
                ['source'][batch_segment_indexes]
