...
from models import (Cnn_5layers_AvgPooling, Cnn_9layers_MaxPooling, 
    Cnn_9layers_AvgPooling, Cnn_13layers_AvgPooling, Cnn_9layers_AvgPooling_Emb, Cnn_9layers_AvgPooling_GCNEmb, Cnn_9layers_AvgPooling_N2VEmb)
...
def get_model(classes_num, model_type, args):
    ...
    if model_type == "Cnn_9layers_AvgPooling_GCNEmb":
        with open(args.graph_dir, "rb") as f:
            graph, class_indices = pickle.load(f)
        model = Model(classes_num, graph, class_indices)
    elif model_type == "Cnn_9layers_AvgPooling_N2VEmb":
        model = Model(classes_num, args.n2v_dir)
    else:
...
def train(args):
    ...
    if model_type == "Cnn_9layers_AvgPooling_GCNEmb":
        ...
        graph_marker = args.n2v_dir[args.n2v_dir.rfind("/")+1:-8]
        model_marker = f"{model_type}_{graph_marker}"
    else:
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--n2v_dir', type=str, default="", help='Directory of n2v numpy result.')
